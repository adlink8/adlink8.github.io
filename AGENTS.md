...
**The sky is CSS-only** — no images, no WebGL (custom.css §26, markup in footer.html). Dark: the REAL moon phase — `extend_head.html` computes the lunar age (synodic month from the 2000-01-06 new moon) and sets `data-moon-quarter` / `data-moon-term` / `--moon-ts` / `--moon-illum` on `<html>`; §26's layered-disc geometry (night side + lit half + terminator ellipse) renders tonight's actual moon, static, no animation — plus ~36 JS-generated meteors and two twinkling star layers. Light: sun-ray cones with sway, 7 clouds (shaded undersides), mist bands, a 50/50 blue/white sky split by a drifting wavy `clip-path` border, JS-generated dust motes (daylight's meteors), a bird flock crossing every ~78s, and a time-of-day tint — `extend_head.html` sets `data-daytime` (dawn/day/dusk/night), dawn/dusk warm the sky and sun (§36). The Scorpius constellation (§30) stays hidden by default — only the Oct 25 birthday egg (gold) or the typed word `scorpio` reveals it. The starry-night canvas is the single sanctioned exception to CSS-only. The sky never restarts on navigation: `extend_head.html` sets `--sky-clock` (negative wall-clock delay) pre-paint and §35 applies it to every long-cycle sky animation — new sky animations must consume `var(--sky-clock)`; page-to-page fades use cross-document view transitions.
